...
$categories_stmt = $pdo->query("SELECT * FROM categories WHERE slug != 'all' ORDER BY id");
...
<body>
    <div class="main">
        ...
        <section class="catalog">
            ...
            <div class="category-nav">
                <a href="catalog.php?category=all" class="category-link <?= $category_slug == 'all' ? 'active' : '' ?>">Все</a>
                <?php foreach ($categories as $cat): ?>
                    <a href="catalog.php?category=<?= $cat['slug'] ?>"
                        class="category-link <?= $category_slug == $cat['slug'] ? 'active' : '' ?>">
    ...
    <script>
        ...
        // Добавление/удаление из корзины с синхронизацией кнопки
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        // Меняем кнопку и скрытое поле
                        if (currentAction === 'add') {
                            // Был добавлен → теперь статус "в корзине"
                            formData.set('cart_action', 'remove');
                            button.classList.add('btn-remove');
                            button.innerHTML = '🗑️ Удалить из корзины';
                        } else {
                            // Был удалён → теперь статус "не в корзине"
